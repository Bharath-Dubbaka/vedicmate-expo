...
const chatSlice = createSlice({
  ...
  reducers: {
    addMessage: (state, action) => {
      ...
      console.log(
        `[CHAT SLICE] addMessage to ${matchId}: "${message.text?.substring(
          0,
          30
        )}..."`
      );
    ...
    removeMessage: (state, action) => {
      ...
      if (state.messagesByMatch[matchId]) {
        state.messagesByMatch[matchId] = state.messagesByMatch[matchId].filter(
          (m) => m._id !== messageId
        );
      }
    },
...
export const { addMessage, removeMessage, setCurrentMatch, clearChat } = chatSlice.actions;
