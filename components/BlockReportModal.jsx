...
export default function BlockReportModal({ visible, matchId, userId, userName, onClose, onBlocked }) {
  ...
  const handleReport = async () => {
    if (!selectedReason) {
      Alert.alert('Select a reason', 'Please choose why you\'re reporting this profile.');
      return;
    }
    setLoading(true);
    try {
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={reset}>
      <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: COLORS.modalBg }}>
        ...
        <View style={{
          ...
          {screen === 'main' ? (
            <>
              <Text style={{ fontFamily: FONTS.headingBold, fontSize: 16, color: COLORS.textPrimary, letterSpacing: 2, marginBottom: SPACING.lg, textAlign: 'center' }}>
                {userName}
              </Text>
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontFamily: FONTS.bodyMedium, fontSize: 15, color: COLORS.rose, marginBottom: 2 }}>
                    Block {userName}
                  </Text>
              ...
              >
                <Text style={{ fontSize: 22 }}>⚑</Text>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontFamily: FONTS.bodyMedium, fontSize: 15, color: COLORS.textPrimary, marginBottom: 2 }}>
                    Report {userName}
                  </Text>
                  <Text style={{ fontFamily: FONTS.body, fontSize: 12, color: COLORS.textSecondary }}>
              ...
              <TouchableOpacity
                ...
                onPress={reset}
              >
                <Text style={{ fontFamily: FONTS.bodyMedium, fontSize: 15, color: COLORS.textSecondary }}>Cancel</Text>
              </TouchableOpacity>
            </>
          ) : (
            <>
              <TouchableOpacity onPress={() => setScreen('main')} style={{ marginBottom: SPACING.md }}>
                <Text style={{ fontFamily: FONTS.bodyMedium, fontSize: 14, color: COLORS.gold }}>← Back</Text>
              </TouchableOpacity>

              <Text style={{ fontFamily: FONTS.headingBold, fontSize: 16, color: COLORS.textPrimary, letterSpacing: 2, marginBottom: 4 }}>
                REPORT {userName.toUpperCase()}
              </Text>
              <Text style={{ fontFamily: FONTS.body, fontSize: 13, color: COLORS.textSecondary, marginBottom: SPACING.lg }}>
              ...
              {REPORT_REASONS.map((r) => (
                ...
                >
                  <View style={{
                    ...
                    borderRadius: 10,
                    borderWidth: 2,
                    borderColor: selectedReason === r.key ? COLORS.gold : COLORS.border,
                    backgroundColor: selectedReason === r.key ? COLORS.gold : 'transparent',
                    alignItems: 'center',
              ...
              >
                {loading ? (
                  ...
                  <Text style={{ fontFamily: FONTS.bodyBold, fontSize: 15, color: '#fff' }}>Submit Report</Text>
                )}
              </TouchableOpacity>
            </>
          )}
